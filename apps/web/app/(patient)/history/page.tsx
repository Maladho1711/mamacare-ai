    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-7 gap-1.5" role="grid" aria-label="Calendrier 30 jours">
          ...
          {cells.map((cell) => {
            ...
            const bg        = level ? (LEVEL_BG[level] ?? 'bg-gray-200') : 'bg-gray-200';
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] text-gray-500 font-medium">{cell.date.getDate()}</span>
              </button>
            );
          })}
        </div>
        ...
        {selected?.response && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3">
            ...
            {selected.response.ai_analysis && (
              <p className="text-sm text-gray-600 leading-relaxed">{selected.response.ai_analysis}</p>
            )}

            {selected.response.triggered_rules.length > 0 && (
              <div>
                <p className="text-xs text-gray-400 uppercase font-semibold tracking-wide mb-1">Signes détectés</p>
                <div className="flex flex-wrap gap-1.5">
                  {selected.response.triggered_rules.map((r) => (
                    <span key={r} className="text-xs px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full">
        ...
        {/* ── Rappels grossesse ── */}
        {pregnancyWeek !== null && (
          <div className="bg-[#FDE8F3] border border-pink-200 rounded-xl p-4">
            <p className="text-xs font-semibold text-[#C9177A] uppercase tracking-wide mb-1">
              Rappels consultations
            </p>
            <p className="text-sm text-[#A81266] leading-relaxed">
              {nextConsultations(pregnancyWeek)}
            </p>
            {expectedTerm && (
              <p className="text-sm text-[#A81266] mt-1">
